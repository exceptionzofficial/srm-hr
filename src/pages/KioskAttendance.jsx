...
const KioskAttendance = () => {
    ...
    const handleGenerate = async () => {
        try {
            ...
            console.log("Generating Kiosk Report with params:", params);
            ...
            if (response.success) {
                ...
                // FILTER FOR KIOSK ONLY
                reportData = reportData.filter(emp => isKiosk(emp));

                if (response.type === 'range') {
                    setReport(reportData);
                    // Recalculate summary data might be needed if API returns aggregate
                    // But here API returns list of stats per employee, so filtering list works.
                    setSummaryData({
    ...
    const downloadCSV = () => {
        try {
            ...
            const fileName = isRange ? `kiosk_attendance_summary_${startDate}_to_${endDate}.csv` : `kiosk_attendance_report_${date}.csv`;
